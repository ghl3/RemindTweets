...
class TestMentionParsing extends JUnitSuite {
  ...
  @Test def parsingH() {
    ...
    val data = Map("what"->"go to sleep", "time"->"midnight", "repeat"->"day")
    assertParsedProperly(mention, data, List("when"))
  }
  ...
  @Test def parsingI() {
    ...
    val data = Map("what"->"eat my lunch", "time"->"noon", "repeat"->"Tuesday")
    assertParsedProperly(mention, data, List("when"))
  }
